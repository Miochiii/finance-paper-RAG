...
## 漂移检测（最小 e-detector，E1/E2/E5 雏形）

E0 选出的代理指标要真正用于监控，需要**序贯检测器**。`edetector.py` 是 E-detector 框架
（Shin, Ramdas, Rinaldo 2023）的最小实现：有界变量构造 + 凸混合 + 显式阈值 1/α。

```bash
python edetector.py                       # ARL 验证 + EDD + 融合对比（几分钟，无需 GPU）
python edetector.py --m-strategy q60,q85,q100   # 扫描 m → ARL/EDD 权衡曲线（E5）
python edetector.py --alpha 0.001 --T 3000 --reps 500   # 更贴近生产的误报水平
python edetector.py --no-inject           # 只做变前 ARL 验证
python edetector.py --reps 200 --reps-edd 200 --inject 0.2,0.4 --window 20 \
                    --m-strategy q85 --edd-horizon 300   # E1 对照矩阵（推荐口径）
```

构造（原文 §5.2，只要求变前 μ ≤ m，**不要求 iid**）：

```
L_n(λ) = 1 + λ(X_n/m − 1)；M^CU_n = L_n · max(M^CU_{n−1}, 1)；M_n = Σω·M^CU_n；报警 M_n ≥ 1/α
```

实验与已知结论：
- **ARL ≥ 1/α**（阈值显式、非渐近）：`m` 是合法上界时成立；用**样本均值**当上界会违反 μ ≤ m 前提，
  实测 ARL 明显低于 1/α；
- **取最大不合法（Remark 3.1 实证）**：`max` 融合在合法的 `mix` 保持沉默时仍会报警
  （同一批流上实测 18% vs 0%），即工程直觉最自然的做法会误报膨胀；`min` 合法但保守（EDD 长得多）；
- **m 的保守性权衡（E5）**：`q60/q85/q100` 或 `mean/p95/hoeffding` 扫描可见——
  m 越保守 ARL 越安全、EDD 越长，过保守会导致"永远不报警"（ARL > T）；
- **α 的工程含义**：ARL ≥ 1/α，α=0.05 意味着平均每 20 步就可能误报一次（生产太吵），
  实际部署通常取 1e-3 量级；因此 EDD 实验用 `--alpha-edd 0.001`。

### 单指标 vs 多指标混合（E1：漂移类型未知时的鲁棒性）

生产里没人告诉你"这次是哪种退化"，所以真正要回答的是：**盯着某一个代理指标的检测器，
能不能扛住打在别的指标上的漂移？** 做法是把 E0 选出的代理组成联合流（`--e1-proxies`，
默认 `ans_refusal,ret_doc_hhi,ret_unique_docs`），**同一批流 + 同一次注入**配对比较，
并且漂移**只打在其中一个指标上**：

```bash
python edetector.py --reps 200 --T 2000 --reps-edd 200 --inject 0.2,0.4 \
                    --window 20 --m-strategy q85 --edd-horizon 300
```

Δ=0.4 的注入结果（α_edd=1e-3，20 步滑窗；口径 = 检出率 / EDD 步数，检出只认变点后 300 步内的报警）：

| 漂移只打在 | 单指标#0 拒答 | 单指标#1 集中度 | 单指标#2 分散度 | **全指标混合** | 取最大（不合法） |
|---|---|---|---|---|---|
| #0 拒答 | **1.00 / 10** | 0.00 / — | 0.00 / — | **1.00 / 13** | 1.00 / 10 |
| #1 集中度 | 0.00 / — | **1.00 / 35** | 0.00 / — | **1.00 / 39** | 1.00 / 27 |
| #2 分散度 | 0.00 / — | 0.00 / — | **1.00 / 27** | **1.00 / 30** | 1.00 / 21 |

**对角线结构就是 E1 的结论**：单指标检测器只在"漂移正好打在它身上"时有效，
打偏了是**彻底失效（检出率 0.00）而不是变慢**——9 个「检测器 × 漂移目标」组合里只有 3 个
（对角线）能检出；凸混合（命题 2.3 保证合法）在三种漂移下**3/3 全部检出**，
代价只是相对"事先知道漂移打在哪"的 oracle 单指标慢 11~25%。`max` 看着更快，但它在变前就压不住
误报（另一组设置下实测报警率 0.05 → 0.38，5.7 倍膨胀），**不合法的"快"不能要**；
`min` 合法但过于保守，几乎从不报警。

**混合也有代价，且要诚实报告**：漂移幅度小（Δ=0.2）时，混合被不敏感的指标稀释——
集中度那一列单指标检出 0.26、混合只有 0.15（`max` 反而 0.44）。也就是说
**融合换来的是"漂移类型未知时的兜底"，不是"免费加速"**；漂移太弱时稀释效应会压过鲁棒性收益。

实现上的四个坑：
- 多指标必须**联合重采样**（`bootstrap_rows` 对"行"重采样一次，各指标取同一批行），
  各自独立重采样会人为削弱指标间相关性、把混合效果估得过于乐观；
- 每个指标要用**自己的** m（合法性是逐指标 μ_k ≤ m_k），共用一个大 m 会把敏感指标一起埋掉；
- **检出要有视界**（`--edd-horizon`）：不设视界时，一个对该漂移本就无感的检测器会因零假设侧的
  偶发误报被记成"检出"，出现过「检出率 0.01、EDD 493 步」的假象，还会把对照矩阵的对角线弄脏；
- 对照必须是**盯对指标的那个单指标**（oracle 基线），跨漂移目标取"最快"是错误对照。

实现细节（踩过的坑，都写进代码注释与测试）：归一化尺度只用**校准段**定（否则偷看变点后的漂移值）；
切分前**打乱顺序**（标注集行序是 v1→v2 批次而非时间序，按行序切会把批次差异当漂移，
实测直接违反 μ ≤ m 前提）；重复流维度不能压掉；e-value 乘法要截断防溢出；
**各实验段用独立的随机数发生器**——共用一个 rng 时，E1 的校准集位置会随前面实验的 `--reps/--T`
变化，同一 seed 下能把结论从"全部报警"翻成"一条都不报警"。

输出：`results/edetector_<时间戳>.csv`（逐配置 ARL/EDD，含"m ≥ 变前均值"合法性标记）、
`results/edetector_series_<时间戳>.csv`（示例轨迹，可直接画图）、
`log/E-detector_最小验证_<日期>.md`（报告）。
