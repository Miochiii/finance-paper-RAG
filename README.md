...
```bash
python probe_validity.py                  # hmm 40 题（检索侧代理需本地 BGE）
python probe_validity.py --no-embed       # 跳过嵌入类指标，无需 GPU
python probe_validity.py --methods all    # 回答侧代理扩到 5 种分块（n=200）
```

口径：13 个代理 × 7 个真值（judge 正确性/忠实性、gold 召回、结论证据充分性、证据缺口率、
答案↔标准答案语义相似度、综合质量分）；Spearman ρ + 10000 次配对 bootstrap 区间 +
同族 Holm 校正，并给出**该样本量下的最小可检测效应**（n=40 时 |ρ| ≥ 0.31，n=200 时 0.14）。

自建语料上的结果（hmm 40 题 / 回答侧 200 行）：

| 代理 → 真值 | ρ | Holm p | n | 判定 |
|---|---|---|---|---|
| 检索 top1/平均相似度 → judge 忠实性 | **+0.481** | 0.022 | 40 | ✅ 纳入 |
| 拒答标志 → gold 文献被召回 | **−0.474** | 0.026 | 40 | ✅ 纳入 |
| 拒答标志 → judge 正确性 | −0.282 | 0.001 | 200 | ✅ 纳入 |
| 检索文档多样性 → judge 正确性 | −0.361 | 0.221 | 40 | ⚠️ 方向与直觉相反 |
| 引用密度 → 综合质量分 | −0.211 | 0.030 | 200 | ⚠️ 引用越多评分越低 |
| 结论证据充分性（真值） | — | — | 40 | 无任何代理显著相关 |

两条结论：**① 有效代理确实存在**（检索相似度、拒答行为），可以作为检测器输入；
**② 主要瓶颈是真值的区分度**——judge 分数天花板明显（135/200 满分）、证据充分性 96% 挤在 1.0，
唯一高区分度真值是答案相似度（160 个不同取值）。因此扩标注（40 → 120，门槛 0.31 → 0.18）
与提高真值分辨率是后续工作的前置条件，而不是可选项。
