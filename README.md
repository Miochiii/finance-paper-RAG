...
```bash
python edetector.py --reps 300 --T 3000 --reps-edd 200 --inject 0.2 --window 20 --m-strategy q85
```

Δ=0.2 的注入结果（检出率 / EDD 步数，α_edd=1e-3，20 步滑窗）：

| 漂移只打在 | 单指标#0 拒答 | 单指标#1 集中度 | 单指标#2 分散度 | **全指标混合** | 取最大（不合法） |
|---|---|---|---|---|---|
| #0 拒答 | **1.00 / 84** | 0.00 / — | 0.00 / — | **1.00 / 94** | 1.00 / 65 |
| #1 集中度 | 0.00 / — | **1.00 / 180** | 0.00 / — | **1.00 / 208** | 1.00 / 137 |
| #2 分散度 | 0.00 / — | 0.00 / — | **1.00 / 193** | **1.00 / 220** | 1.00 / 158 |

**对角线结构就是 E1 的结论**：单指标检测器只在"漂移正好打在它身上"时有效，
打偏了是**彻底失效（0.00）而不是变慢**；凸混合（命题 2.3 保证合法）在三种漂移下**全部检出**，
代价只是延迟增加 10~15%。`max` 看着更快，但它在变前就压不住误报（ARL 实测 0.05 → 0.38 报警率，
5.7 倍膨胀），**不合法的"快"不能要**；`min` 合法但过于保守，EDD 长到几乎检不出。

实现上的两个坑：多指标必须**联合重采样**（`bootstrap_rows` 对"行"重采样一次，各指标取同一批行），
各自独立重采样会人为削弱指标间相关性、把混合效果估得过于乐观；每个指标要用**自己的** m
（合法性是逐指标 μ_k ≤ m_k），共用一个大 m 会把敏感指标一起埋掉。
