...
## 4. 参考结果（38 篇 / 40 题，2026-08-28，检索 + 生成完整跑批）
...
3. **生成质量稳定**：正确性 4.1~4.4/5、忠实性 4.65~4.88/5——答案严格基于检索证据、不编造；EM/F1 低是因为标准答案是短句、生成答案是长段落，字符级匹配天然困难，生成质量以 judge 打分为准；
4. **分块方式无统计显著差异**：90 组配对检验中仅 discourse vs hmm 的 F1 一项 Wilcoxon p=0.039（多重比较下视为噪声，t 检验 p=0.196 不显著），其余全部 p > 0.05。分块方式不是质量瓶颈，默认 hmm（BIC 自适应 + 块缓存加速）可以放心保留。
